...
const AddShows = () => {
    ...
    const [dateTimeSelction, setDateTimeSelection] = useState({})
    ...
    const handleSubmit = async () => {
        try {
            setAddingShow(true)
            if (!selectedMovie || Object.keys(dateTimeSelction).length === 0 ||
                !showPrice) {
                return toast('missing require fields');
            }
            const showsInput = Object.entries(dateTimeSelction).map(([date, time]) => ({ date, time }));
    ...
    return nowPlayingMovies.length > 0 ? (
        <>
            ...
            <div className='mt-8'>
                ...
                px-3 py-2 rounded-md'>
                    ...
                    <input min={0} type="number" value={showPrice} onChange={(e) =>
                        ...
                        placeholder='Nhập giá tiền' className='outline-none' />
                </div>
            </div>
            ...
            {Object.keys(dateTimeSelction).length > 0 && (
                <div className='mt-6'>
                    <h2 className='mb-2'>Thời gian đã chọn</h2>
                    <ul className='space-y-3'>
                        {Object.entries(dateTimeSelction).map(([date, times]) => (
                            <li key={date}>
            ...
            hover:bg-primary/90 transition-all cursor-pointer'>Thêm Phim</button>
        </>
    ) : <Loading />
}
